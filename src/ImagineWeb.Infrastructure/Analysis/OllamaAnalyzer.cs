...
public class OllamaAnalyzer : IOllamaAnalyzer
{
    private readonly OllamaClient _client;
    private readonly ILogger<OllamaAnalyzer> _logger;
    private readonly OllamaConfig _config;

    public OllamaAnalyzer(OllamaClient client, ILogger<OllamaAnalyzer> logger, IOptions<OllamaConfig> config)
    {
        _client = client;
        _logger = logger;
    ...
    public async Task<Result<AnalysisResult>> AnalyzePageAsync(string url, string title, string content, string? sessionContext, CompetitorContext? competitors, EnrichmentData? enrichment, CancellationToken ct)
    {
        ...
        try
        {
            var response = await _client.GenerateAsync(prompt, _config.Model, Phase1Schema, maxTokens: 1536, numCtx: null, ct);
            var result = ParsePhase1Response(response);

            _logger.LogInformation("Phase1 {Url}: Score={Score}, Opportunity={Opp}",
                url, result.ProfitScore, result.OpportunityType);

            return Result<AnalysisResult>.Success(result);
        }
        catch (OperationCanceledException) when (ct.IsCancellationRequested) { throw; }
        catch (OllamaRequestException ex) when (ex.InnerException is OperationCanceledException)
        {
            _logger.LogWarning("Ollama request timed out for {Url}: {Message}", url, ex.Message);
            return Result<AnalysisResult>.Failure($"Analysis timed out: {ex.Message}");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to analyze page {Url}", url);
            return Result<AnalysisResult>.Failure($"Analysis failed: {ex.Message}");
        }
    }
    ...
    public async Task<Result<AnalysisResult>> DeepAnalyzeAsync(AnalysisResult phase1, string url, string title, CompetitorContext? competitors, string? domainContext, CancellationToken ct)
    {
        ...
        try
        {
            var response = await _client.GenerateAsync(prompt, _config.Model, Phase2Schema, maxTokens: 2048, numCtx: null, ct);
            var enriched = ParsePhase2Response(response, phase1);

            _logger.LogInformation("Phase2 {Url}: Feasibility={Feas}, Effort={Eff}, Reward={Rew}",
                url, enriched.FeasibilityScore, enriched.EstimatedEffort, enriched.EstimatedReward);
        ...
        {
            _logger.LogWarning(ex, "Phase2 analysis failed for {Url}, keeping Phase1 results", url);
    ...
    public Task<bool> IsAvailableAsync(CancellationToken ct) => _client.IsAvailableAsync(ct);

    public async Task<string> RawPromptAsync(string prompt, CancellationToken ct)
    {
        try
        {
            return await _client.GenerateAsync(prompt, ct);
        }
        catch (OperationCanceledException) when (ct.IsCancellationRequested) { throw; }
        catch (Exception ex)
        {
    ...
    public async Task<List<int>> PreScreenResultsAsync(List<(string Title, string Url, string Snippet)> candidates, int topN, CancellationToken ct)
    {
        ...
        try
        {
            ...
            return indices?.Where(i => i >= 0 && i < candidates.Count).Distinct().ToList() ?? [];
        }
        catch (OperationCanceledException) when (ct.IsCancellationRequested) { throw; }
        catch (Exception ex)
        {
    ...
    private static AnalysisResult ParsePhase2Response(string response, AnalysisResult phase1)
    {
